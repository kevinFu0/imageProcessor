...
public class ImageProcessorModelImplTest {
  ...
  public void testFlipHorizontally() {
    int channels = imagePModel.getPixelAt(0, 0).length;
    int[][][] image = new int[imagePModel.getImageHeight()][imagePModel.getImageWidth()][channels];
    for (int r = 0; r < imagePModel.getImageHeight(); r++) {
      for (int c = imagePModel.getImageWidth() - 1; c >= 0; c--) {
        image[r][c] = imagePModel.getPixelAt(r, c);
      }
    }
    imagePModel.flipHorizontally();

    int[][][] flippedImage = new int[imagePModel.getImageHeight()][imagePModel.getImageWidth()]
            [channels];
    for (int r = 0; r < imagePModel.getImageHeight(); r++) {
      for (int c = 0; c < imagePModel.getImageWidth(); c++) {
        flippedImage[r][c] = imagePModel.getPixelAt(r, c);
      }
    }
