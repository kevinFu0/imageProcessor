...
This iteration of the image processing adds a graphical user interface and the ability
to create histograms based on image data. The histogram functionality was added through the addition of an
ImageProcessorModelHistogram interface that extends the EnhancedImageProcessorModel. This interface has one
additional method called getHistogram that returns a histogram of the given component of the given image as an array.
This interface is implemented by ImageProcessorHistogramImpl which uses encapsulation to add this new
functionality to an existing EnhancedImageProcessorModel.

The graphical user interface was added via a new ImageGuiView interface and the ImageGuiViewImpl class
which is responsible for displaying the image being worked on and the corresponding histograms for red,
green, blue, and intensity components as well as providing messages to the user near the top of the window.
The buttons on the left expose the possible operations a user can perform (load, save, blur, sharpen,
greyscale, darken, brighten, flip horizontally, flip vertically, sepia, and visualize intensity, value,
luma, red, green, or blue).

With this view GUI comes a Features interface whose methods are called from the view through actionevents.
The new ImageGuiController that implements this (in addition to the ImageProcessController interface) is
responsible for communicating with both the GUI and the model.

Citations: The River.jpg image in the res folder was provided by myself, Jaden McMiller. The other images are cited
below.
